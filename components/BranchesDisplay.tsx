...
const BranchesDisplay = ({ attainableBranches }: BranchesDisplayProps) => {
  ...
  const collegesWithBranches = attainableBranches.filter(
    (collegeData) => collegeData.branches.length > 0,
  );
  ...
  return (
    <section className="mt-8">
      ...
      {collegesWithBranches.map((collegeData) => (
        <div
              ...
              >
                <ul className="grid gap-4 sm:grid-cols-2">
                  {collegeData.branches.map((branch, index) => (
                    <li
                      key={index}
                      className="flex items-start justify-between gap-3 border-2 border-slate-600 bg-slate-900 px-4 py-3 hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-none shadow-[4px_4px_0px_#64748b] transition-all"
                    >
                      <span className="min-w-0 wrap-break-word text-sm font-bold uppercase text-white sm:text-base">
                        {branch.name}
                      </span>
